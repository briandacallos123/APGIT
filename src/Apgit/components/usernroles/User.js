...
export default function DepartmentTablesRow({ row, selected, onSelectRow, onViewRow, onEditRow, onDeleteRow }) {
  ...
  const { name, email, department, designation, id, img, phone, role, shift, status } = row;
  //   const { id, name, email, phone, designation, img, department, role, shift, status } = row;
  ...
  return (
    <TableRow hover selected={selected}>
      ...
      {/* <TableCell padding="checkbox">
        <Stack direction="row" sx={{ alignItems: 'center' }}>
          <img src={img} alt="" style={{ borderRadius: '50%', marginRight: '.5rem', width: '50px' }} />
          <Typography variant="subtitle2" noWrap sx={{ textTransform: 'capitalize' }}>
            {name}
          </Typography>
        </Stack>
