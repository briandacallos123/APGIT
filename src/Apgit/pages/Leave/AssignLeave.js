...
import { _assign } from '../../../_mock/customTable/leave';
// import {  } from '../../../_mock';
// import {  } from '../../../_mock';
// components
...
// import Assign from '../../components/LeaveTables/Assigns';
...
const SERVICE_OPTIONS = _assign.reduce(
  (arr, currentItem) => {
    if (!arr.includes(currentItem.department)) {
      arr.push(currentItem.department);
    }
...
const TABLE_HEAD = [
  { id: 'invoiceNumber', label: 'Department', width: 1000 },
  { id: 'createDate', label: 'Type', align: 'left', width: 1000 },
  { id: 'days', label: 'Days', align: 'center', width: 500 },
...
export default function AssignLeave() {
  ...
  const [tableData, setTableData] = useState(_assign);
  ...
  const getActive = () => {
    const newData = _assign.filter((item) => item.status === 'Active');
...
function applySortFilter({
  ...
  if (filterService !== 'all') {
    tableData = tableData.filter((item) => item.department === filterService);
  }
